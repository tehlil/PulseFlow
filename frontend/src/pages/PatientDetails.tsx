...
import { 
  ArrowLeft, Activity, Calendar, Sparkles, Heart, Edit, Trash2 
} from 'lucide-react';
...
export default function PatientDetails() {
  ...
  const [visitForm, setVisitForm] = useState({ reason: '', notes: '', visitDate: new Date().toISOString().split('T')[0] });
  ...
  const visitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setVisitForm({ reason: '', notes: '', visitDate: new Date().toISOString().split('T')[0] });
    },
  });
  ...
  const handleDeleteAssessment = (assessmentId: string) => {
    if (window.confirm('Are you sure you want to delete this clinical assessment record? This will soft-delete the chart and invalidate its AI risk predictions.')) {
      deleteAssessmentMutation.mutate(assessmentId);
    }
  };

  if (isLoading) {
    return <div className="p-8 text-center text-primary-muted animate-pulse">Loading patient chart folder...</div>;
  }

  if (error || !data) {
    return (
      <div className="p-6 bg-healthcare-rose/10 border border-healthcare-rose/20 rounded-xl text-healthcare-rose text-sm">
        Failed to load patient chart data. The record may have been soft-deleted or you do not have permission.
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div className="space-y-2">
          <Link to="/patients" className="text-xs text-primary-muted hover:text-white flex items-center space-x-1">
          ...
          <p className="text-xs text-primary-muted">
            {data.gender} • {age} years old • DOB: {new Date(data.dateOfBirth).toLocaleDateString(undefined, { dateStyle: 'long' })}
          </p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-background-card border border-border p-6 rounded-xl shadow-sm flex flex-col space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xs font-bold text-white uppercase tracking-wider">Vitals Trends Panel</h2>
                <p className="text-[10px] text-primary-muted">Historical timelines of blood pressure and glucose check recordings.</p>
              </div>
            ...
            <div className="h-60 w-full">
              {vitalsTrendData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={vitalsTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <div className="bg-background-card border border-border rounded-xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-border">
              <h2 className="text-xs font-bold text-white uppercase tracking-wider">Vitals Log Trail</h2>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border/60 text-xs">
                  {data.assessments.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="py-8 text-center text-primary-muted">
                        No physical vital checks recorded.
                      </td>
                    </tr>
                  ) : (
                    data.assessments.map((a: any) => (
                      <tr key={a.id} className="hover:bg-background-hover/20 transition-colors">
                        <td className="py-3.5 px-6 text-white font-medium">
                          {new Date(a.date).toLocaleDateString(undefined, { dateStyle: 'short', timeStyle: 'short' })}
                        </td>
                        <td className="py-3.5 px-6 font-mono">
                          {a.bloodPressureSystolic && a.bloodPressureDiastolic ? `${a.bloodPressureSystolic}/${a.bloodPressureDiastolic}` : '—'}
                        </td>
                        <td className="py-3.5 px-6 font-mono">{a.heartRate || '—'}</td>
                        <td className="py-3.5 px-6 font-mono">{a.bloodSugar || '—'}</td>
                        <td className="py-3.5 px-6 font-mono text-healthcare-cyan">{a.oxygenSaturation ? `${a.oxygenSaturation}%` : '—'}</td>
                        <td className="py-3.5 px-6 font-mono">{a.temperature ? `${a.temperature}°C` : '—'}</td>
                        <td className="py-3.5 px-6 font-mono">
                          {a.weight || a.height ? `${a.weight || '—'} kg / ${a.height || '—'} cm` : '—'}
                        </td>
                        <td className="py-3.5 px-6 font-mono">{a.bmi || '—'}</td>
                        <td className="py-3.5 px-6">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            a.status === 'COMPLETED' ? 'bg-healthcare-emerald/10 text-healthcare-emerald' :
                            a.status === 'REVIEWED' ? 'bg-healthcare-cyan/10 text-healthcare-cyan' :
                            a.status === 'ARCHIVED' ? 'bg-border text-primary-muted' :
        ...
        <div className="space-y-6">
          ...
          <div className="bg-background-card border border-border p-6 rounded-xl shadow-sm flex flex-col space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-center space-x-2 text-healthcare-amber">
              ...
              <h2 className="text-xs font-bold text-white uppercase tracking-wider">AI Predictive Risk</h2>
            </div>

            {latestPrediction ? (
              <div className="space-y-4">
                <div className="flex items-baseline justify-between">
                  <p className="text-[10px] text-primary-muted font-bold uppercase">{latestPrediction.predictionType} RISK</p>
                  <span className="text-xs text-primary-muted">Confidence: {Math.round(latestPrediction.confidenceScore * 100)}%</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-3 py-1 rounded text-[10px] font-bold ${
                    latestPrediction.riskCategory === 'CRITICAL' ? 'bg-red-950 text-red-500 border border-red-900/50' :
                    latestPrediction.riskCategory === 'HIGH' ? 'bg-healthcare-rose/10 text-healthcare-rose' :
                    latestPrediction.riskCategory === 'MODERATE' ? 'bg-healthcare-amber/10 text-healthcare-amber' :
                ...
                <div className="space-y-2 pt-2 border-t border-border">
                  <p className="text-[10px] font-semibold text-white uppercase">Key Risk Contributors</p>
                  <div className="space-y-1.5">
                    {(latestPrediction.contributingFactors as any[]).map((f, i) => (
                      <div key={i} className="flex justify-between items-center text-xs">
                        <span className="text-primary-muted">{f.factor}</span>
                        <span className="text-healthcare-rose font-semibold">+{f.weight} pts</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-border space-y-1">
                  <p className="text-[10px] font-semibold text-white uppercase">Suggested Directives</p>
                  <p className="text-xs text-primary-muted leading-relaxed italic">
                    "{(latestPrediction.recommendedActions as string[])[0] || 'Observe status changes.'}"
                  </p>
                </div>
                
                <Link to="/predictions" className="text-xs text-healthcare-cyan hover:underline font-semibold block text-center pt-2">
              ...
              <div className="text-center py-6 space-y-2 text-xs text-primary-muted">
                ...
                <p className="text-[10px]">Assessments generate predictions in the background.</p>
              </div>
            )}
          </div>
          ...
          {/* Visits Timeline */}
          <div className="bg-background-card border border-border p-6 rounded-xl shadow-sm flex flex-col space-y-4">
            <h2 className="text-xs font-bold text-white uppercase tracking-wider">Consultation History</h2>
            
            <div className="space-y-4 max-h-64 overflow-y-auto pr-1">
              {data.visits.length === 0 ? (
                <p className="text-xs text-primary-muted text-center py-4">No doctor consultation visits logged.</p>
              ) : (
                data.visits.map((v: any) => (
                  <div key={v.id} className="relative pl-5 border-l border-border space-y-1 text-xs">
                    {/* Circle marker */}
                    ...
                    <div className="flex justify-between items-baseline">
                      <span className="font-semibold text-white truncate max-w-[120px]">{v.reason}</span>
                      <span className="text-[10px] text-primary-muted font-mono">
                        {new Date(v.visitDate).toLocaleDateString(undefined, { dateStyle: 'short' })}
                      </span>
                    </div>
                    <p className="text-primary-muted text-[11px] leading-relaxed truncate">{v.notes || 'No notes.'}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {showVisitModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleVisitSubmit} className="bg-background-card border border-border max-w-md w-full rounded-xl p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Log Consultation Visit</h3>
              <p className="text-[10px] text-primary-muted">Create a consultation encounter log for clinical tracking.</p>
            </div>
            
            <div className="space-y-4 text-xs">
              <div className="space-y-1.5">
                <label className="font-semibold text-white uppercase">Visit Date</label>
                <input
                  type="date"
                  required
              ...
              <div className="space-y-1.5">
                <label className="font-semibold text-white uppercase">Reason for Consultation</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1.5">
                <label className="font-semibold text-white uppercase">Encounter Notes</label>
                <textarea
                  rows={3}
